...
func enterInstanceEnvironment(alias, instancePath string) error {
	...
	env := os.Environ()
	env = append(env, fmt.Sprintf("MAVBAKE_INSTANCE_PATH=%s", instancePath))
	...
	case "zsh":
		...
		ps1 := fmt.Sprintf("%%F{cyan}%%Bmavbake%%b%%f %%F{blue}❯%%f %%F{75}instance%%f %%F{blue}❯%%f %%F{159}%%B%s%%b%%f %%F{243}%%~%%f %%F{blue}❯%%f ", alias)
	...
	case "bash":
		...
		ps1 := fmt.Sprintf("\\[\\033[1;36m\\]mavbake\\[\\033[0m\\] \\[\\033[34m\\]❯\\[\\033[0m\\] \\[\\033[94m\\]instance\\[\\033[0m\\] \\[\\033[34m\\]❯\\[\\033[0m\\] \\[\\033[1;96m\\]%s\\[\\033[0m\\] \\[\\033[90m\\]\\w\\[\\033[0m\\] \\[\\033[34m\\]❯\\[\\033[0m\\] ", alias)
		env = append(env, "PS1="+ps1)
		shellArgs = append(shellArgs, "--norc", "--noprofile")
	default:
		// For other shells, use simple ANSI codes with $PWD
		ps1 := fmt.Sprintf("\033[1;36mmavbake\033[0m \033[34m❯\033[0m \033[94minstance\033[0m \033[34m❯\033[0m \033[1;96m%s\033[0m \033[90m$PWD\033[0m \033[34m❯\033[0m ", alias)
		env = append(env, "PS1="+ps1)
	}
...
var instanceCmd = &cobra.Command{
	...
	Short: "Executes command on a specific mavbake instance or enters the instance environment",
	Long: `Proxies the command to the specified mavbake instance by setting the appropriate path.
...
var listCmd = &cobra.Command{
	...
	Long:  "Lists all available mavbake instances found in the default directory.",
	RunE: func(cmd *cobra.Command, args []string) error {
		bbDir := cli.BBdir
		if bbDir == "" {
		...
		// Default instance exists if there is any other directory in the /mavbake except instances
