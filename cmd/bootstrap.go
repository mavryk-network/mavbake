...
const snapshotBaseURL = "https://snapshots.api.mavryk.network"
...
func (m bootstrapModel) View() string {
	...
	switch m.state {
	case stateQuickSelect:
		title := "🥯 MavBake Node Bootstrap"
		if m.nodePath != "" {
			title = fmt.Sprintf("🥯 MavBake Node Bootstrap [%s]", m.nodePath)
		}
...
func runSnapshotSelector(nodePath string) snapshotSelection {
	...
	if err != nil {
		log.Error("Error running snapshot selector:", "error", err)
...
var bootstrapNodeCmd = &cobra.Command{
	...
	Short: "Bootstraps MavBake's Mavryk node.",
...
to help you choose the appropriate snapshot for your needs.`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if nodePath != "" {
			log.Info("Bootstrapping node at:", "node_path", nodePath)
		}
		log.Info("Bootstrapping from:", "snapshot_source", snapshotSource)
		if blockHash != "" {
			log.Info("Block hash:", "hash", blockHash)
		}
